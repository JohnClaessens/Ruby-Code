...
class Quiz
  ...
  def display_instructions
    ...
    loop do
      ...
      print "Would you like to know more about the grading scale? (y/n): "

      moreinst = STDIN.gets  #Collect the player's response
      moreinst.chop!  #Remove one character from the end of the string
      
      #call the display scale method
      if moreinst == "y"
        SQ.disp_scale
        else
        end

      break if moreinst == "y" || moreinst == "n" 
  ...
  def disp_scale
      Console_Screen.cls  #Clear the display area
      puts " In order to pass the Superman Movie Quiz you must get"
      puts " at least 6 questions right."
      puts " The grading scale is as follows:"
      puts "  6 correct answers: Good Citizen" 
      puts "  7 correct answers: Good Citizen" 
      puts "  8 correct answers: Side Kick"
      puts "  9 correct answers: Side Kick"
      puts " 10 correct answers: Superhero"
  ...
  def determine_grade
    ...
    if $noRight >= 6 then
      ...
      puts "You have earned a rank of: Side Kick" if $noRight == 9
      ...
      puts "For more information reguarding Superman Movies please visit "
...
if answer == "n"  #See if the player elected not to take the quiz
  ...
  until $goagain == "n" do #create loop to allow player to retake the quiz
    
  $noRight = 0 # reset the Number of questions right back to zero
  ...
  "d")
      ...
      if $noRight > 5 
         $goagain = "n"   
            ...
            Console_Screen.cls  #Clear the display area
            #Prompt the player for permission to restart the quiz
            puts  " You have only answered " + $noRight.to_s + " questions right and will fail." +
                  " Would youlike to try again? (y/n)"
                  
            $goagain = STDIN.gets  #Collect the player's response
            $goagain.chop!  #Remove one character from the end of the string

            break if $goagain == "y" || $goagain == "n" 
