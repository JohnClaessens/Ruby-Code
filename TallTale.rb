...
class Tale
  ...
  attr_accessor :monster, :villain, :object, :place, :location, :friend, :mom, :dad
...
if $answer == "n" #See if the player elected not to play
  ...
  #Prompt the player to provide some input
  print %Q{Type the name of a friend. (Press Enter)\n\n: }
  friend = STDIN.gets  #Force the player to press Enter to continue
  friend.chomp!  #Remove any extra characters appended to the string
  
  Console_Screen.cls  #Clear the display area

  
  #Prompt the player to provide some input
  print %Q{Type the name of your mom. (Press Enter)\n\n: }
  mom = STDIN.gets  #Force the player to press Enter to continue
  mom.chomp!  #Remove any extra characters appended to the string
  
  Console_Screen.cls  #Clear the display area
  
  #Prompt the player to provide some input
  print %Q{Type the name of your dad. (Press Enter)\n\n: }
  dad = STDIN.gets  #Force the player to press Enter to continue
  dad.chomp!  #Remove any extra characters appended to the string
  ...
  named #{friend}, #{mom} and #{dad}. These children were known far and 
  ...
  #{location} sent word to #{friend}, #{mom}, and #{dad} that their help 
  ...
  Story.P4 = %Q{  Immediately, #{friend}, #{mom} and #{dad} flew into 
  action. #{friend} threw a #{object} and hit #{villain} in order to get 
  his attention. At the same time #{mom} and #{dad} crept up behind him. 
  #{mom} then threw a rope around the feet of #{villain}, momentarily 
  tying him up and allowing #{dad} to move in and spray #{villain} with 
  ...
  life in #{location} was back to normal. #{friend}, #{mom} and #{dad} 
  ...
  Story.P5 = %Q{  This game was created by Jerry Lee Ford, Jr. and 
  modified by John Claessens as part of a programming assignment 
  at Bellevue University. The school's website is www.bellevue.edu.  (Press Enter)

  }
  
  Console_Screen.cls  #Clear the display area
  puts "  Thank you for providing inputs for the story,\n  Press Enter to begin the story." #transitional message
